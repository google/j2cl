 ...
 *
 * @UnsafeVariance, due to an intentional parameter mismatch.
 *
 * Extracted the relevant parts from org.jetbrains.kotlin.backend.jvm.lower.BridgeLowering.kt
 *
 ...
 */
internal class BridgeLowering(val context: JvmBackendContext) :
  ...
  override fun visitClass(declaration: IrClass): IrStatement {
    ...
    if (bridgeTargets.isEmpty()) return super.visitClass(declaration)
    ...
    //   // Inline class (implementing 'MutableCollection<T>', where T is Int or an inline class
    // mapped to Int)
    //   // can contain a static replacement for a function 'remove', which forces value parameter
    // boxing
    //   // in order to avoid signature clash with 'remove(int)' method in 'java.util.List'.
    //   // We should rewrite this static replacement as well ('remove' function itself is handled
    // during special bridge processing).
    ...
    //     original != null &&
    // context.defaultMethodSignatureMapper.shouldBoxSingleValueParameterForSpecialCaseOfRemove(original)
    //   }
    //   if (remove != null) {
  ...
  private fun IrSimpleFunction.isPotentialBridgeTarget(): Boolean {
    ...
    // Note that this includes the static replacements for inline class functions (which are static,
    // but have
    ...
    // directly inherited from an interface.
    // This is sound, since we do not generate type-specialized versions of fake overrides and if
    // the method
    // were to override several interface methods the frontend would require a separate
    // implementation.
  ...
  private fun makeLastParameterNullable(irFunction: IrSimpleFunction) {
    ...
    val newValueParameter =
      oldValueParameter.copyTo(irFunction, type = oldValueParameter.type.makeNullable())
    irFunction.valueParameters = irFunction.valueParameters.dropLast(1) + newValueParameter
    irFunction.body?.transform(
      VariableRemapper(mapOf(oldValueParameter to newValueParameter)),
      null
    )
  }
  ...
  private fun createBridges(irClass: IrClass, irFunction: IrSimpleFunction) {
    ...
    if (specialBridge != null) {
      ...
      blacklist +=
        irFunction.allOverridden().flatMapTo(arrayListOf()) { it.getSpecialBridgeSignatures() }

      fun getSpecialBridgeTargetAddingExtraBridges(): IrSimpleFunction {
        // We only generate a special bridge method if it does not clash with a final method in a
        // superclass or the current method
        if (
        ...
        if (irFunction.isFakeOverride) {
          // If irFunction is a fake override, we replace it with a stub and redirect all calls to
          // irFunction with calls to the stub
          // instead. Otherwise, we'll end up calling the special method itself and get into an
          // infinite loop.
          bridgeTarget =
            when {
              ...
              irFunction.modality != Modality.FINAL -> {
                // If we have a non-abstract, non-final fake-override we need to put in an
                // additional bridge which uses
                ...
                val superBridge =
                  SpecialBridge(
                    ...
                    methodInfo =
                      specialBridge.methodInfo?.copy(
                        argumentsToCheck = 0
                      ), // For potential argument boxing
                    isFinal = false,
                  )

                // The part after '?:' is needed for methods with default implementations in
                // collection interfaces:
                // MutableMap.remove() and getOrDefault().
                val superTarget =
                  overriddenFromClass.takeIf { !it.isFakeOverride || !specialBridge.isOverriding }
                    ?: specialBridge.overridden

                if (superBridge.signature == superTarget.jvmMethod) {
                  // If the resulting bridge to a super member matches the signature of the bridge
                  // callee,
                ...
                // then we will not override it in a subclass and we do not need to generate an
                // additional stub method.
                irFunction
              }
            }
        ...
        // Java bridges is commented out.
        //
        // // Add special bridge with unsubstituted signature (as generated by old JVM back-end) if
        // required.
        // // According to KT-40277 we generate special bridges for 'get', 'getOrDefault', and
        // 'remove' with substituted return type.
        // // However, this introduces performance regressions and changes in behavior if cases like
        // KT-48945
        // // (where a Kotlin class implements a read-only collection interface and extends a Java
        // collection class).
        ...
        //   irClass.functions.none { it.isClashingWithPotentialBridge(irFunction.name,
        // unsubstitutedSpecialBridge.signature) }
        // ) {
        ...
        if (
          irClass.functions.none {
            it.isClashingWithPotentialBridge(irFunction.name, specialBridge.signature)
          }
        ) {
      ...
      if (!irFunction.isFakeOverride && specialBridge.methodInfo != null) {
        irFunction.rewriteSpecialMethodBody(
          targetMethod,
          specialBridge.signature,
          specialBridge.methodInfo!!
        )
      }
      ...
      // Java bridges is commented out.
      //
      // // For generic special bridge methods we need to generate bridges for generic overrides
      // coming from Java or Kotlin interfaces.
      ...
      // Do not generate bridge methods for abstract methods which do not override a special bridge
      // method.
      // This matches the behavior of the JVM backend, but it does mean that we generate superfluous
      // bridges
      // for abstract methods overriding a special bridge for which we do not create a bridge due
      // to,
      // e.g., signature clashes.
      return
    }

    // For concrete fake overrides, some bridges may be inherited from the super-classes.
    // Specifically, bridges for all
    // declarations that are reachable from all concrete immediate super-functions of the given
    // function. Note that all such bridges are
    // guaranteed to delegate to the same implementation as bridges for the given function, that's
    // why it's safe to inherit them.
    //
    ...
    if (irFunction.isFakeOverride) {
      for (overriddenSymbol in irFunction.overriddenSymbols) {
        ...
        override.allOverridden().filter { !it.isFakeOverride }.mapTo(blacklist) { it.jvmMethod }
      }
    }
  ...
  private fun IrSimpleFunction.isClashingWithPotentialBridge(
    ...
    (!this.isFakeOverride || this.modality == Modality.FINAL) &&
      this.name == name &&
      this.jvmMethod == signature
  ...
  private fun IrSimpleFunction.getSpecialBridgeSignatures(): List<Method> {
    ...
    // in Java
    // classes, we may generate special bridges in interfaces for methods annotated with
    // @JvmDefault.
    ...
    if (this.parentAsClass.isInterface || this.isFromJava()) return emptyList()
    val specialBridge = this.specialBridgeOrNull ?: return emptyList()
  ...
  private fun IrClass.addAbstractMethodStub(irFunction: IrSimpleFunction) =
    addFunction {
      ...
      .apply {
        // If the function is a property accessor, we need to mark the abstract stub as a property
        // accessor as well.
        // However, we cannot link in the new function as the new accessor for the property, since
        // there might still
        // be references to the original fake override stub.
        copyCorrespondingPropertyFrom(irFunction)
        dispatchReceiverParameter = thisReceiver?.copyTo(this, type = defaultType)
        valueParameters =
          irFunction.valueParameters.map { param -> param.copyTo(this, type = param.type) }
  ...
  private fun IrClass.addBridge(bridge: Bridge, target: IrSimpleFunction): IrSimpleFunction =
    addFunction {
      ...
      .apply {
        ...
        // Otherwise, it might throw ClassCastException when downcasting bridge argument to expected
        // type.
        // See KT-49765
        body =
          if (target.isThrowingStub()) {
        ...
        if (!bridge.overridden.returnType.isTypeParameterWithPrimitiveUpperBound()) {
          // The generated bridge method overrides all of the symbols which were overridden by its
          // overrides.
          // This is technically wrong, but it's necessary to generate a method which maps to the
          // same signature.
          // In case of 'fun foo(): T', where 'T' is a type parameter with primitive upper bound
          // (e.g., 'T : Char'),
          // 'foo' is mapped to 'foo()C', regardless of its overrides.
          val inheritedOverrides =
            bridge.overriddenSymbols.flatMapTo(mutableSetOf()) { it.owner.overriddenSymbols }
          val redundantOverrides =
            inheritedOverrides.flatMapTo(mutableSetOf()) {
  ...
  private fun IrSimpleFunction.isThrowingStub(): Boolean {
    if (
      this.origin != IrDeclarationOrigin.IR_BUILTINS_STUB &&
  ...
  private fun IrClass.addSpecialBridge(
    ...
    addFunction {
        ...
        origin =
          if (specialBridge.isSynthetic) IrDeclarationOrigin.BRIDGE
          else IrDeclarationOrigin.BRIDGE_SPECIAL
        name = Name.identifier(specialBridge.signature.name)
        returnType =
          specialBridge.substitutedReturnType?.eraseToScope(target.parentAsClass)
      ...
      .apply {
        ...
        if (
          context.defaultMethodSignatureMapper.shouldBoxSingleValueParameterForSpecialCaseOfRemove(
            this
          )
        ) {
  ...
  private fun IrSimpleFunction.rewriteSpecialMethodBody(
    ...
    // If there is an existing function that would conflict with a special bridge signature, insert
    // the special bridge
    ...
    if (specialOverrideSignature == ourSignature) {
      ...
      if (shouldGenerateParameterChecks) {
        ...
        context.createIrBuilder(symbol).run {
          body = irBlockBody {
            // Change the parameter types to be Any? so that null checks are not generated. The
            // checks
            ...
            argumentsToCheck.forEach {
              ...
              if (!parameterType.isNullable()) {
                val newParameter =
                  it.copyTo(this@rewriteSpecialMethodBody, type = parameterType.makeNullable())
      ...
      // If the signature of this method will be changed in the output to take a boxed argument
      // instead of a primitive,
      // rewrite the argument so that code will be generated for a boxed argument and not a
      // primitive.
      valueParameters =
        valueParameters.mapIndexed { i, p ->
          if (
            AsmUtil.isPrimitive(context.defaultTypeMapper.mapType(p.type)) &&
              ourSignature.argumentTypes[i].sort == Type.OBJECT
          ) {
  ...
  private fun IrBuilderWithScope.parameterTypeCheck(
  ...
  private fun IrValueParameter.copyWithTypeErasure(
    ...
    copyTo(
      ...
      // Currently there are no special bridge methods with vararg parameters, so we don't track
      // substituted vararg element types.
      varargElementType = varargElementType?.eraseToScope(visibleTypeParameters),
      // If the parameter has a default value, replace it with a stub, as if this function is coming
      // from an external dependency.
      // Otherwise it can lead to all sorts of problems, for example this default value can
      // reference private functions from another
  ...
  private fun IrBuilderWithScope.irCastIfNeeded(
    expression: IrExpression,
    to: IrType
  ): IrExpression =
    if (expression.type == to || to.isAny() || to.isNullableAny()) expression
    else irImplicitCast(expression, to)
