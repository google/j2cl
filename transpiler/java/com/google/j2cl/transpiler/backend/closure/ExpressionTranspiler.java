...
public final class ExpressionTranspiler {
  public static void render(
    ...
    new Visitor(environment, sourceBuilder).renderNoParens(expression);
  }

  public static void renderMultiVariableDeclaration(
      List<? extends Expression> variableDeclarationExpressions,
      ClosureGenerationEnvironment environment,
      SourceBuilder sourceBuilder) {
    new Visitor(environment, sourceBuilder)
        .renderMultiVariableDeclaration(variableDeclarationExpressions);
  }
  ...
  private static class Visitor extends AbstractVisitor {
    ...
    Visitor(ClosureGenerationEnvironment environment, SourceBuilder sourceBuilder) {
    ...
    private String getJsDoc(FunctionExpression expression) {
      ...
      if (!typeParameterTypeDescriptors.isEmpty()) {
        sb.append(" @template ");
        sb.append(
            typeParameterTypeDescriptors.stream().map(TypeVariable::getName).collect(joining(",")));
      }
    ...
    public boolean enterVariableDeclarationExpression(
      ...
      renderMultiVariableDeclaration(Collections.singletonList(variableDeclarationExpression));
      return false;
    }

    void renderMultiVariableDeclaration(List<? extends Expression> variableDeclarationExpressions) {
      if (variableDeclarationExpressions.isEmpty()) {
        return;
      }
      ...
      for (Expression expression : variableDeclarationExpressions) {
        sourceBuilder.append(separator);
        separator = ", ";
        renderVariableDeclaration((VariableDeclarationExpression) expression);
      }
    }

    private void renderVariableDeclaration(
        VariableDeclarationExpression variableDeclarationExpression) {
      ...
      // This is a variable declaration, render without parens.
      renderNoParens(variable);

      if (variableDeclarationExpression.getInitializer() != null) {
        sourceBuilder.append(" = ");
        // It is safe to render without parentheses even in a case like 'int a = b = c' because
        // '=' is right associative.
        renderNoParens(variableDeclarationExpression.getInitializer());
      }
    }
