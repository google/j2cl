...
public class ByteArrayOutputStream extends OutputStream {
  ...
  /**
   * Constructs a new ByteArrayOutputStream with a default size of 32 bytes. If more than 32 bytes
   * are written to this instance, the underlying byte array will expand.
   */
  ...
  public void close() throws IOException {
    /**
     * Although the spec claims "A closed stream cannot perform output operations and cannot be
     * reopened.", this implementation must do nothing.
     */
    super.close();
  }
  ...
  /**
   * Returns the contents of this ByteArrayOutputStream as a string. Any changes made to the
   * receiver after returning will not be reflected in the string returned to the caller.
   *
  ...
  /**
   * Returns the contents of this ByteArrayOutputStream as a string. Each byte {@code b} in this
   * stream is converted to a character {@code c} using the following function: {@code c ==
   * (char)(((hibyte & 0xff) << 8) | (b & 0xff))}. This method is deprecated and either {@link
   * #toString()} or {@link #toString(String)} should be used.
   *
   * @param hibyte the high byte of each resulting Unicode character.
   * @return this stream's current contents as a string with the high byte set to {@code hibyte}.
   ...
   *
   * @param charsetName a string representing the encoding to use when translating this stream to a
   *     string.
  ...
  /**
   * Writes the specified byte {@code oneByte} to the OutputStream. Only the low order byte of
   * {@code oneByte} is written.
   *
