...
internal class CollectionStubMethodLowering(val context: JvmBackendContext) : ClassLoweringPass {
  ...
  override fun lower(irClass: IrClass) {
    ...
    // We don't need to generate stub for existing methods, but for FAKE_OVERRIDE methods with
    // ABSTRACT modality,
    ...
    val (abstractMethods, nonAbstractMethods) =
      irClass.functions.partition { it.modality == Modality.ABSTRACT && it.isFakeOverride }
    ...
    for (stub in methodStubsToGenerate) {
      ...
      // Generated stub might still override some abstract member(s), which affects resulting method
      // signature.
      val overriddenAbstractMethods =
        abstractMethodsByNameAndArity[stubNameAndArity].orEmpty().filter {
          isEffectivelyOverriddenBy(it, stub)
        }
      stub.overriddenSymbols += overriddenAbstractMethods.map { it.symbol }

      // Some stub members require special handling.
      // In both 'remove' and 'removeAt' cases there are no other member functions with same name in
      // built-in mutable collection
      ...
      when (stub.name.asString()) {
        ...
        "removeAt" -> {
          ...
          //      We've checked that corresponding 'removeAt(Int)' member function is not present in
          // the class
          //      (if it IS present, special bridges for 'remove(I)' would be generated later in
          // BridgeLowering).
          //      We can't add 'removeAt' here, because it would be different from what old back-end
          // generates
          ...
          val removeIntFun =
            createRemoveAtStub(stub, stub.returnType, IrDeclarationOrigin.IR_BUILTINS_STUB)
          irClass.declarations.add(removeIntFun)

          // MODIFIED BY GOOGLE
          // We don't need the bridge method as we don't have the same binary constraints
          // and it would be a duplicate definition.

          // val removeIntBridgeFun = createRemoveAtStub(stub, context.irBuiltIns.anyNType,
        ...
        else -> irClass.declarations.add(stub)
      }
    }
  }
  ...
  private fun isEffectivelyOverriddenBy(
    ...
    // (because members of Kotlin built-in collection classes don't have type parameters of their
    // own),
  ...
  private fun areTypeParametersEquivalent(
    ...
    overrideFun.typeParameters.zip(parentFun.typeParameters).all { (typeParameter1, typeParameter2)
      ->
      typeParameter1.superTypes.zip(typeParameter2.superTypes).all { (supertype1, supertype2) ->
  ...
  // Compute a substitution map for type parameters between source class (Mutable Collection
  // classes) to
  // target class (class currently in lowering phase), this map is later used for substituting type
  // parameters in generated functions
  private fun computeSubstitutionMap(
    ...
    // of target class
    // Perform type substitution along searching, then use the type arguments obtained from the most
    // specific type
    // for type substitution.
    val readOnlyClassType =
      getAllSubstitutedSupertypes(targetClass).findMostSpecificTypeForClass(readOnlyClass.symbol)
    val readOnlyClassTypeArguments =
      (readOnlyClassType as IrSimpleType).arguments.mapNotNull { (it as? IrTypeProjection)?.type }

    if (
      readOnlyClassTypeArguments.isEmpty() ||
        readOnlyClassTypeArguments.size != mutableClass.typeParameters.size
    ) {
  ...
  private fun generateRelevantStubMethods(irClass: IrClass): List<IrSimpleFunction> {
    val classStubFuns =
      collectionStubComputer.stubsForCollectionClasses(irClass).flatMap {
        it.createStubFuns(irClass)
      }
    if (classStubFuns.isEmpty()) return classStubFuns

    val alreadyPresent =
      computeStubsForSuperClasses(irClass)
  ...
  private fun computeStubsForSuperClass(superClass: IrClass): List<StubsForCollectionClass> {
    ...
    if (superClassStubs.isEmpty() || superClass.modality != Modality.ABSTRACT)
      return superClassStubs

    // An abstract superclass might contain an abstract function declaration that effectively
    // overrides a stub to be generated,
    // and thus have no non-abstract stub.
    // This calculation happens for each abstract class multiple times. TODO memoize.

    val abstractFunsByNameAndArity =
    ...
    return superClassStubs.map {
      ...
      // Resulting stub can be different from a stub created in substitution context of the
      // "current" class
      // in case of (partially) specialized generic superclass.
      val substitutionMap =
        computeSubstitutionMap(it.readOnlyClass.owner, it.mutableClass.owner, superClass)
      val filteredCandidates =
        it.candidatesForStubs.filter { candidateFun ->
  ...
  private fun Collection<IrType>.findMostSpecificTypeForClass(classifier: IrClassSymbol): IrType {
    ...
    return types.first { type -> types.all { other -> type.isSubtypeOfClass(other.classOrNull!!) } }
  }
