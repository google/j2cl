...
public class FrontendUtils {
  ...
  public static List<String> expandSourcePathSourceJarEntries(
    ...
    for (String sourceFilePath : sourceFilePaths) {
      if (sourceFilePath.endsWith(SRCJAR_EXTENSION)) {
        try {
          expandedSourceFilePaths.addAll(
              FrontendUtils.expandZipFile(sourceFilePath, srcjarContentDir));
        } catch (IOException e) {
  ...
  private static List<String> expandZipFile(String sourceJarPath, Path srcjarTempDir)
      throws IOException {
    List<String> expandedEntries = new ArrayList<>();
    // Extract the sourceJar.
    ZipFiles.unzipFile(new File(sourceJarPath), srcjarTempDir.toFile());
        ...
        new SimpleFileVisitor<Path>() {
          ...
          public FileVisitResult visitFile(Path path, BasicFileAttributes attrs)
            ...
            if (path.toString().endsWith(JAVA_EXTENSION)) {
              expandedEntries.add(path.toAbsolutePath().toString());
            }
