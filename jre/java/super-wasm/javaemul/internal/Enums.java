...
class Enums {
  ...
  public static BoxedIntegerEnum boxInteger(int value, Class<?> clazz) {
    if (isNull(value)) {
      return null;
    }
    return JsEnumCache.getCachedInteger(clazz, value);
  }

  private static class BoxedIntegerEnum extends BoxedLightEnum {
    final int value;

    private BoxedIntegerEnum(int value, Class<?> clazz) {
      super(clazz);
      this.value = value;
    }
  ...
  public static BoxedStringEnum boxString(String value, Class<?> clazz) {
    ...
    return JsEnumCache.getCachedString(clazz, value);
  }

  private static class BoxedStringEnum extends BoxedLightEnum {
    final String value;

    private BoxedStringEnum(String value, Class<?> clazz) {
      super(clazz);
      this.value = value;
    }
  ...
  private static final class JsEnumCache {
    private static Map<String, Object> jsEnumsCache = new HashMap<>();

    @HasNoSideEffects
    public static BoxedIntegerEnum getCachedInteger(Class<?> enumClass, int value) {
      return (BoxedIntegerEnum)
          jsEnumsCache.computeIfAbsent(
              enumClass.getName() + "$" + value, key -> new BoxedIntegerEnum(value, enumClass));
    }

    @HasNoSideEffects
    public static BoxedComparableIntegerEnum getCachedComparableInteger(
        Class<?> enumClass, int value) {
      return (BoxedComparableIntegerEnum)
          jsEnumsCache.computeIfAbsent(
              enumClass.getName() + "$" + value,
              key -> new BoxedComparableIntegerEnum(value, enumClass));
    }

    @HasNoSideEffects
    public static BoxedStringEnum getCachedString(Class<?> enumClass, String value) {
      return (BoxedStringEnum)
          jsEnumsCache.computeIfAbsent(
              enumClass.getName() + "$" + value, key -> new BoxedStringEnum(value, enumClass));
    }
  }
}
