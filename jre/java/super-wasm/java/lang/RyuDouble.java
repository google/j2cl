...
final class RyuDouble {
  ...
  static {
    BigInteger mask =
        BigInteger.valueOf(1).shiftLeft(POW5_QUARTER_BITCOUNT).subtract(BigInteger.ONE);
    BigInteger invMask =
        BigInteger.valueOf(1).shiftLeft(POW5_INV_QUARTER_BITCOUNT).subtract(BigInteger.ONE);
    for (int i = 0; i < POS_TABLE_SIZE; i++) {
      ...
      if (i < POW5_SPLIT.length) {
        for (int j = 0; j < 4; j++) {
          POW5_SPLIT[i][j] =
              pow.shiftRight(pow5len - POW5_BITCOUNT + (3 - j) * POW5_QUARTER_BITCOUNT)
                  .and(mask)
                  .intValueExact();
        }
      }
      ...
      if (i < POW5_INV_SPLIT.length) {
        ...
        for (int k = 0; k < 4; k++) {
          if (k == 0) {
            POW5_INV_SPLIT[i][k] =
                inv.shiftRight((3 - k) * POW5_INV_QUARTER_BITCOUNT).intValueExact();
          } else {
            POW5_INV_SPLIT[i][k] =
  ...
  public static String doubleToString(AbstractStringBuilder sb, double value) {
    ...
    // Double.toString semantics requires using scientific notation if and only if outside this
    // range.
  ...
  /**
   * Compute the high digits of m / 5^i / 2^j such that the result is accurate to at least 9 decimal
