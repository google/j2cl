...
JS and packages it into a jsunit_test() rule for testing. Java library deps
might be needed for reference resolution.
...
import com.google.j2cl.junit.J2clTestInput;

/**
 * J2CL java tests.
 *
 * @author Class generated by build rule j2cl_test.
 */
@J2clTestInput({%s})
public class %s {
}
...
def _getClassName(name):
  return name.title() + "__generated_j2cl_test_input"


def _getFilename(name):
  return _getClassName(name) + ".java"
...
def _generate_suite(name, package, srcs):
  ...
  class_name = _getClassName(name)
  java_code = (
      _TEMPLATE %
      (BUILD_FILE, "j2cl_test", package,
       ",".join(full_qualified_classes),
       class_name))
  _write_file(name=class_name, content=java_code)


def j2cl_test(**kwargs):
  ...
  # it in transpilation
  kwargs["srcs"] = kwargs[
      "srcs"] + [_getFilename(base_name)]
  if not "deps" in kwargs:
    kwargs["deps"] = []
  kwargs["deps"] = (
      kwargs["deps"] +
      ["//junit/java/com/google/j2cl/junit:internal_junit_annotations"]
  )
