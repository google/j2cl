...
"""Diffs obfuscated optimized integration test JS with current CL changes."""
...
def main():
  ...
  js_file_path = repo_util.get_obfuscated_optimized_test_file(test_name)
  ...
  if synced_to_cl < MIN_DIFF_CL:
    print(
        "  must be synced to atleast CL %s to be able to do obfuscated diff." %
        MIN_DIFF_CL)
    return
  ...
  print ("  blaze building obfuscated opt JS for '%s' in the managed repo" %
         test_name)
  repo_util.build_obfuscated_optimized_test(test_name, cwd=MANAGED_GOOGLE3_PATH)
  ...
  print ("  blaze building obfuscated opt JS for '%s' in the live repo" %
         test_name)
  repo_util.build_obfuscated_optimized_test(test_name)
