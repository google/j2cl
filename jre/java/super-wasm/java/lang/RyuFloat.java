...
final class RyuFloat {
  ...
  private static final long LOG2_5_NUMERATOR =
      (long) (LOG2_5_DENOMINATOR * (Math.log(5) / Math.log(2)));
  ...
  static {
    ...
    BigInteger maskInv =
        BigInteger.valueOf(1).shiftLeft(POW5_INV_HALF_BITCOUNT).subtract(BigInteger.ONE);
  ...
  public static String floatToString(AbstractStringBuilder sb, float value) {
    ...
    // Float.toString semantics requires using scientific notation if and only if outside this
    // range.
    ...
    int output =
        dv + ((dv == dm && !(dmIsTrailingZeros && even)) || (lastRemovedDigit >= 5) ? 1 : 0);
  ...
  private static int pow5bits(int e) {
    return e == 0
        ? 1
        : (int) ((e * LOG2_5_NUMERATOR + LOG2_5_DENOMINATOR - 1) / LOG2_5_DENOMINATOR);
  }

  /**
   * Returns the exponent of the largest power of 5 that divides the given value, i.e., returns i
   * such that value = 5^i * x, where x is an integer.
   */
  ...
  /**
   * Compute the exact result of [m * 5^(-e_2) / 10^q] = [m * 5^(-e_2 - q) / 2^q] = [m * [5^(p -
  ...
  /**
   * Compute the exact result of [m * 2^p / 10^q] = [m * 2^(p - q) / 5 ^ q] = [m * [2^k / 5^q] /
