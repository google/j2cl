...
def _compile(
    ...
    keep_j2kt_debug_outputs = ctx.var.get("keep_j2kt_debug_outputs", "false").lower() in ("true", "1")
    ...
    if has_srcs_to_transpile:
        ...
        if enable_sourcemap_composition:
            ...
            if not keep_j2kt_debug_outputs:
                # Exclude J2CL readable source maps (`.mappings`), which are invalidated during
                # source map composition.
                args.add("-excludeTargetExtension", ".mappings")

                # Exclude intermediate `.kt` source files generated by J2KT.
                args.add("-excludeTargetExtension", ".kt")
            ctx.actions.run(
