...
public class PruningResultsTest {
  ...
  public void testFooImplFileIsCorrectlyPruned() {
    ImmutableList<String> prunedLinesChecks =
        ImmutableList.of(
            "constructor() {",
            "$create__() {",
            "$ctor__com_google_j2cl_tools_rta_pruningresults_Foo__() {",
            "m_unusedMethod__() {",
            // TODO(b/116175766): getter and setter should be removed
            "Foo.$f_unusedStaticField__com_google_j2cl_tools_rta_pruningresults_Foo;");

    // Because the input is generated by j2cl and can change, first assert the content of the file
    // contains the lines we expect to be pruned.
    prunedLinesChecks.forEach(
        line -> assertWithMessage("Input file is incorrect:").that(fooFile.content).contains(line));

    String contentAfterPruning = j2clMinifier.minify(fooFile.path, fooFile.content);

    prunedLinesChecks.forEach(
        line ->
            assertWithMessage("Line was not pruned:")
                .that(contentAfterPruning)
                .doesNotContain(j2clMinifier.minify(line)));

    // assert we don't change the number of lines for not breaking source map.
    assertThat(numberOfLinesOf(fooFile.content)).isEqualTo(numberOfLinesOf(contentAfterPruning));
  }
