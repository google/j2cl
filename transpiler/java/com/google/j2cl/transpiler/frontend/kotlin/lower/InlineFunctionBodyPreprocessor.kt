...
internal class InlineFunctionBodyPreprocessor(
  ...
  fun preprocess(irElement: IrFunction): IrFunction {
    ...
    // Copy IR.
    val result =
      irElement.transform(copier, data = null).let {
        // Just a performance optimization. In most cases nothing is to be done in this postprocess,
        // so no reason to traverse the tree.
    ...
    object : DeepCopyIrTreeWithSymbols(symbolRemapper, typeRemapper) {
      ...
      override fun visitCall(expression: IrCall): IrCall {
        return super.visitCall(expression).also {
          // We can't do it right now, because we need to return IrCall, and postprocessor for
          // Native
          // want to return IrConstructorCall. In principle this could be done by changing return
          // type in
